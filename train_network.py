# to use this have added few paramaters to work with
# python train_network.py --dataset dataset


# set the matplotlib backend so figures can be saved in the background
import matplotlib
#agg is used to render graphs and stuff
matplotlib.use("Agg")

# import all the necessary packages
from mainsimple.simplenet import SimpleNet
...
#have added some params so that it becomes easier
...
# initialize the optimizer and model in the cmd probably
print("[INFO] compiling model...")
opt = Adam(lr=1e-4, decay=1e-4 / args["epochs"])#learning rate
...
model.save('/home/rusty3699/Desktop/test/test/anishv1.model')
...
idxs = np.random.choice(idxs, size=(49,), replace=False)
images = []
...
for i in idxs:
	...
	color = (238, 138, 238) if "non" in label else (255, 165, 0)
	cv2.putText(output, label, (3, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
		color, 2)
	images.append(output)

# create a montage using 128x128 "tiles" with 5 rows and 5 columns
montage = build_montages(images, (128, 128), (7, 7))[0]
